...
; 32-битный сегмент данных, который отсутствует в памяти
GDT_DS_New	db		0,0,0,0,0,01110010b,01000000b,0
...
; Пункт 3. Команда чтения из сегмента локальной таблицы дескрипторов
; mov CX, 0001110011b ;
; mov GS, CX          ;
; mov CX, GS:[0]      ;
...
; Пункт 2. Выполнить обращение к данным при помощи нулевого селектора
mov CX, 0000000000b ;
MOV GS, CX          ;
mov CX, GS:[0000]   ; считать два байта из начала сегмента

; Пункт 2.2 Обратиться к сегменту данных, который отсутствует в памяти (valid = 0)
; mov CX, 0001111011b ;
; mov GS, CX          ;
; mov CX, GS:[0]      ;
